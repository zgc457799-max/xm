  ...
  return (
    <div className="max-w-6xl mx-auto md:space-y-8 animate-fade-in pb-20 md:pb-12 px-4 md:px-0 mt-4 md:mt-0">
      ...
      <div className="md:hidden flex flex-col gap-4">
        ...
        <div className={`flex items-center justify-between pb-2 border-b ${isDark ? 'border-white/5' : 'border-slate-100'}`}>
           <div>
              <h1 className={`text-xl font-black flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>
                 <Trophy size={20} className={isDark ? "text-blue-400" : "text-blue-500"} />
                 比赛<span className={isDark ? "text-cyan-400" : "text-cyan-500"}>大厅</span>
              </h1>
        ...
        {/* 紧凑版倒计时卡片 */}
        <div className={`${isDark ? 'bg-gradient-to-br from-blue-900/40 to-indigo-900/40 border-blue-500/20' : 'bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-100'} border rounded-2xl p-4 shadow-sm relative overflow-hidden`}>
           <div className={`absolute top-0 right-0 w-24 h-24 ${isDark ? 'bg-cyan-500/10' : 'bg-cyan-500/5'} rounded-full blur-xl -translate-y-1/2 translate-x-1/4`}></div>
           <div className="relative z-10 flex items-center justify-between">
              <div>
                 <div className={`text-[10px] font-black uppercase flex items-center gap-1.5 mb-1 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                    <Timer size={12} /> 距国赛报名截止
                 </div>
                 <div className={`flex gap-1 text-xl font-black font-mono ${isDark ? 'text-white' : 'text-slate-800'}`}>
                    <span>{timeLeft.days.toString().padStart(2, '0')}</span><span className={isDark ? "text-blue-500" : "text-blue-400"}>:</span>
                    <span>{timeLeft.hours.toString().padStart(2, '0')}</span><span className={isDark ? "text-blue-500" : "text-blue-400"}>:</span>
                    <span>{timeLeft.minutes.toString().padStart(2, '0')}</span>
                 </div>
              </div>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center border ${isDark ? 'bg-blue-500/20 text-blue-400 border-blue-500/20' : 'bg-blue-100 text-blue-500 border-blue-200'}`}>
                 <Target size={18} />
              </div>
           </div>
           <p className={`text-[9px] font-bold tracking-wide mt-2 pt-2 border-t ${isDark ? 'text-slate-400 border-white/5' : 'text-slate-500 border-blue-100'}`}>
        ...
        <div className="flex flex-col gap-3">
           {contests.length === 0 ? (
              ...
              contests.map(contest => {
                 ...
                 return (
                    <div key={contest.id} className={`${isDark ? 'bg-slate-900/60 border-white/5' : 'bg-white border-slate-100'} border rounded-2xl p-4 flex flex-col gap-3 shadow-sm`}>
                       <div className="flex items-start gap-3">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl shrink-0 ${isLive ? (isDark ? 'bg-green-500/10 text-green-400' : 'bg-green-50 text-green-500') : isUpcoming ? (isDark ? 'bg-blue-500/10 text-blue-400' : 'bg-blue-50 text-blue-500') : (isDark ? 'bg-white/5 text-slate-500' : 'bg-slate-50 text-slate-400')}`}>
                             <Trophy size={24} />
                          </div>
                          <div className="flex-1">
                             ...
                             <div className="flex items-center gap-2 flex-wrap mb-1.5">
                                ...
                                {contest.isRegistered && !isEnded && (
                                   <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded border ${isDark ? 'bg-blue-500/20 text-blue-400 border-blue-500/20' : 'bg-blue-50 text-blue-500 border-blue-200'}`}>已报名</span>
                                )}
                             </div>
                             <div className={`flex flex-col gap-1 text-[9px] font-bold tracking-wide ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                                <div className="flex items-center gap-1.5">
                                   <Calendar size={10} className={isDark ? "text-slate-500" : "text-slate-400"} />
                                   {formatDate(contest.startTime)} 开始
                                </div>
                                <div className="flex items-center gap-1.5">
                                   <Users size={10} className={isDark ? "text-slate-500" : "text-slate-400"} />
                       ...
                       <div className={`pt-3 w-full border-t ${isDark ? 'border-white/5' : 'border-slate-100'}`}>
                          {isLive ? (
                             ...
                             contest.isRegistered ? (
                                <button disabled className={`w-full py-2.5 rounded-xl text-xs font-black tracking-widest uppercase flex items-center justify-center gap-1 border ${isDark ? 'bg-white/5 text-slate-500 border-white/10' : 'bg-slate-50 text-slate-400 border-slate-200'}`}>
                                   <Clock size={14} /> 等待开始
                                </button>
                             ) : (
                                <button onClick={() => handleRegisterClick(contest.id)} className="w-full py-2.5 bg-blue-600 text-white rounded-xl text-xs font-black tracking-widest uppercase">
